...
EntityManager.prototype.createEntity = function(entityDef, controller)
{
    ...
    {
        logger.warn('Attempting to add entity with the same id "%s"! Returning existing entity.', entityDef.id);

        //==================================================================================
        // FIXME: This isn't actually correct; we shouldn't be doing any of this.
        // Instead, we should simply handle disconnects correctly.
        //==================================================================================

        var entity = this.entities[entityDef.id];

        // Reset the controller to the new controller
        ...
        return Promise.resolve(entityDef.id);
    } // end if
