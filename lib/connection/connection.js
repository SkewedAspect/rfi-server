...
class RFIConnection
{
    ...
    _onLogin(payload, respond)
    {
        if(!payload.account)
        {
            ...
            logger.debug("Account not specified. Rejecting login.");
            return respond({
                confirm: false,
                reason: 'bad_account',
        ...
        {
            logger.debug("Account is already logged in. Rejecting login.");
            return respond({
                confirm: false,
        ...
        models.Character.get(payload.character).getJoin().run()
            .then((character) =>
            {
                ...
                return entityManager.load(character.activeAvatarID)
                    ...
                    .tap(() =>
                    {
                        return require('./manager').sendEntities(this);
                    })
                    .then(() =>
                    {
                        // We respond to the connection, telling it the zone to load.
                        logger.debug("Character selected; sending zone to load.");
                        respond({
                            confirm: true,
                    ...
                    .catch((error) =>
                    {
                        logger.debug("Error loading character: %s", error.stack || error);
                        respond({
                            confirm: false,
                            reason: 'error',
    ...
    sendEvent(eventName, payload)
    {
        if(!this.inGame)
        {
            // Not in the game yet; don't send events.
            return;
        } // end if
