...
WrapperSocket.prototype.event = function(eventName, payload)
{
    ...
    this.emit('outgoing', { type: 'event', eventName: eventName, payload: payload });
...
WrapperSocket.prototype._handleRequest = function()
{
    ...
    if(config.DEBUG_SOCKET)
    {
        logger.debug('Incoming request: "%s" with args:\n%s', args[0], logger.dump(args.slice(1)));
    } // end if
